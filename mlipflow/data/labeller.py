...
from typing import Dict, Union, Tuple, Any, TypedDict, List
...
class GaussianCMLabeller:
    ...
    def __init__(self, references: Dict[str, Atoms], idx_org: Union[list, np.ndarray, slice], 
                 idx_h: Union[list, np.ndarray, slice], sigma_isomer: float = 50.0, 
                 sigma_reaction: float = 10.0, threshold_isomer: float = 0.90, 
                 threshold_reaction: float = 0.85, bond_cutoff: float = 1.45,
                 h2_cutoff: float = 1.0, label_key: str = 'label'):
    ...
    def _gaussian_similarity(self, block1: np.ndarray, block2: np.ndarray, denom: float) -> float:
        dist_sq = np.sum((block1 - block2) ** 2)
        return float(np.exp(-dist_sq / denom))


    def _get_similarity_to_ref(self, isomer_block: np.ndarray, reaction_block: np.ndarray, 
                               ref_label: str) -> Tuple[float, float]:
        ref = self.ref_blocks[ref_label]
        sim_iso = self._gaussian_similarity(isomer_block, ref['isomer_block'], self._denom_isomer)
        sim_rxn = self._gaussian_similarity(reaction_block, ref['reaction_block'], self._denom_reaction)
    ...
    def match_config(self, atoms: Atoms) -> str:
        ...
        if current_label in self.ref_blocks:
            sim_iso, sim_rxn = self._get_similarity_to_ref(isomer_block, reaction_block, current_label)
        ...
        for label in self.ref_blocks:
            ...
            sim_iso, sim_rxn = self._get_similarity_to_ref(isomer_block, reaction_block, label)
    ...
    def match_by_topology(self, atoms: Atoms) -> str:
        ...
        valid_refs = {
            label: data for label, data in self.ref_blocks.items() 
        ...
        if current_label in valid_refs:
            sim_iso = self._gaussian_similarity(isomer_block, valid_refs[current_label]['isomer_block'], self._denom_isomer)
        ...
        for label, ref_data in valid_refs.items():
            ...
            sim_iso = self._gaussian_similarity(isomer_block, ref_data['isomer_block'], self._denom_isomer)
    ...
    def evaluate_frame(self, atoms: Atoms, use_topology: bool = False) -> Dict[str, Any]:
        cm_frame = self._compute_full_cm(atoms)
        isomer_block = self._get_isomer_block(cm_frame)
        ...
        if use_topology:
            ...
            for label, ref_data in self.ref_blocks.items():
                if (ref_data['topology'] != frame_topology) or (frame_topology == ('H2_gas',)):
...
def relabel_configs(in_file: str, reference_configs: str,
                    label_key: str = 'species', use_topology: bool = True, 
                    **kwargs: Any) -> OutputSpec:
    """
    ...
    **kwargs : dict
    ...
    references = {
        at.info[label_key]: at for at in list(ConfigSet(reference_configs))
    }
