...
def _run_autopara_wrappable(list_of_images, calculator, fmax=5e-2, steps=1000,
    ...
    for images in list_of_images:
        ...
        # Instantiate calculator for EACH image to support DyNEB check
        # construct_calculator_picklesafe usually returns a NEW instance if called with tuple
        for at_i, at in enumerate(images):
            at.calc = construct_calculator_picklesafe(calculator)
        ...
        # Check if traj is empty (failed immediately)
        if not traj:
            # If failed immediately, we still want to save the initial state if possible?
            # Or just mark as failed. But NEBGen expects output.
            # If exception occurred, traj might be empty.
            pass
        ...
        if traj:
            ...
            # If traj empty, maybe append None or empty list?
            # But subselect_from_traj might fail.
            # If skip_failures=True, maybe ignore?
            pass
