...
def _sample_autopara_wrappable(
    atoms,
    calculator,
    steps,
    dt,
    integrator="Langevin",
    temperature=None,
    temperature_tau=100.0,
    traj_step_interval=1,
    skip_failures=True,
    results_prefix="last_op__md_",
    verbose=False,
    update_config_type="append",
    traj_select_during_func=lambda _at: True,
...
def _sample_autopara_wrappable_single(
    at,
    at_i,
    calculator,
    steps,
    ...
    if integrator == "Bussi" and Bussi is None:
        raise ImportError(
            "Integrator 'Bussi' selected but ase.md.bussi not found. Upgrade ASE."
        )
    ...
    for t_stage in temperature_use:
        ...
        stage_base = {"timestep": dt * fs, "rng": rng}
    ...
    for stage_i, (stage_kwargs, run_kwargs) in enumerate(
        zip(all_stage_kwargs, all_run_kwargs)
    ):
        if verbose:
            sys.stdout.write(
                f"Running stage: T={stage_kwargs['temperature_K']}K, steps={run_kwargs['steps']}\n"
            )
    ...
    if (len(traj) == 0 or traj[-1] != at) and traj_select_during_func(at):
        at.info["MD_time_fs"] = cur_step * dt
...
def _get_temperature(temperature_use, temperature_tau, steps):
    if temperature_tau is None and (
        temperature_use is not None
        and not isinstance(temperature_use, (float, int, np.floating, np.integer))
    ):
        raise RuntimeError(
            f"NVE (temperature_tau is None) can only accept temperature=float for initial T, got {type(temperature_use)}"
        )
