...
def test_qe_calculator_init(tmp_path):
    ...
    strategy = QECalculator(
        basic_params=str(params_file), pseudopots={}, pseudo_dir=str(pseudo_dir)
    )
...
def test_qe_calculator_get_calculator(mock_prepare_remote, tmp_path):
    ...
    # Test scf
    calc_class, _args, kwargs = strategy.get_calculator(
        job_name="test_job", calc_type="scf"
    )
